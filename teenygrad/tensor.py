...
class Tensor:
    ...
    def __init__(
        ...
        if isinstance(data, LazyBuffer):
            ...
            assert dtype is None or dtype.np is not None, (
    ...
    def assign(self, x) -> Tensor:
        ...
        assert self.shape == x.shape and self.device == x.device, (
        ...
        for i, s in enumerate(orig_slices):
            if s is None:
                ...
                if isinstance(s, int):
                    ...
                    assert isinstance(dim_shape, int), (
    ...
    def gather(self: Tensor, idx: Tensor, dim: int) -> Tensor:
        ...
        assert all(s >= i for s, i in zip(self.shape, idx.shape)), (
    ...
    def cat(self, *args, dim=0) -> Tensor:
        ...
        catargs = [self, *args]
        assert all(t.shape for t in catargs), (
    ...
    def chunk(self, num: int, dim: int = 0) -> List[Tensor]:
        ...
        dim, step = (
            dim + self.ndim if dim < 0 else dim,
            math.ceil(self.shape[dim] / num),
        )
        slice_params = [
    ...
    def _pool(
        ...
        assert all_int(self.shape) and all_int(k_), (
            f"does not support symbolic {self.shape=}, {k_=}"
        )
        s_, d_ = make_pair(stride, len(k_)), make_pair(dilation, len(k_))
        assert len(k_) == len(s_) and len(k_) == len(d_), (
    ...
    def conv2d(
        ...
        if isinstance(padding, (tuple, list)):
            assert len(padding) == 2 * len(HW) or len(padding) == len(HW), (
    ...
    def dot(self, w: Tensor) -> Tensor:
        ...
        assert self.shape[-1] == w.shape[-min(n2, 2)], (
    ...
    def bitcast(self, dtype: DType) -> Tensor:
        assert self.dtype.itemsize == dtype.itemsize, (
