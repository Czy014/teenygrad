...
if __name__ == "__main__":
    ...
    table = []
    for path, subdirs, files in os.walk("teenygrad"):
        for name in files:
            ...
            with tokenize.open(filepath) as file_:
                tokens = [
                    ...
                    if t.type in TOKEN_WHITELIST
                ]
                token_count, line_count = (
                    len(tokens),
                    len(set([t.start[0] for t in tokens])),
                )
    ...
    assert total_line_count < 1000, (
        "TEENYGRAD IS FUCKING TEENY IF YOU GO OVER 1000 LINES IN TEENYGRAD MIGHT AS WELL USE TINYGRAD U FAT FUCK"
    )
